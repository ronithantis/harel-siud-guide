...
// ===== MAIN APP =====
function HarelClaimWizard() {
  const STORAGE_KEY = "harel_claim_draft";
  ...
  const [currentStep, setCurrentStep] = useState(draft ? draft.currentStep || 0 : 0);
  const [formData, setFormData] = useState(draft ? draft.formData || {} : {});
  const [checklist, setChecklist] = useState(draft ? draft.checklist || {} : {});
  const [showAllSteps, setShowAllSteps] = useState(false);
  const [draftLoaded, setDraftLoaded] = useState(!!draft);
  ...
  const clearDraft = () => {
    ...
    setReferralContactConsent(false);
    setReferralNotes("");
  ...
  // Auto-save on every change (only if there's actual data)
  useEffect(() => {
    const hasFormData = Object.keys(formData).some(k => formData[k] && formData[k].toString().trim());
    const hasChecklist = Object.keys(checklist).some(k => checklist[k]);
    if (hasFormData || hasChecklist) {
      saveDraft();
    } else {
      // If everything is empty, remove the draft
      try { localStorage.removeItem(STORAGE_KEY); } catch (e) {}
    }
  }, [formData, checklist, currentStep]);
    ...
    <div style={{ animation: "fadeIn 0.4s ease" }}>
      ...
      >
        ...
        <p style={{
          ...
          (לחברי קופות החולים כללית, מאוחדת ולאומית. חברי מכבי? הביטוח הסיעודי שלכם בהפניקס.)
        </p>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <button
              onClick={() => {
                const videoEl = document.getElementById("welcome-video-wrap");
          ...
          <div id="welcome-video-wrap" style={{
            position: "relative",
            paddingBottom: "56.25%",
            height: 0,
            borderRadius: "12px",
            ...
            <iframe
              src="https://player.vimeo.com/video/1165074541?badge=0&autopause=0&player_id=0&app_id=58479"
              frameBorder="0"
              ...
              title="הגשת תביעת סיעוד — סרטון הסבר"
            />
          </div>
        </div>
        <p
      ...
      <InfoCard icon="🔍" title="איך בודקים אם יש למטופל/ת ביטוח סיעודי?">
        לפני שמתחילים, חשוב לוודא שיש פוליסת סיעוד פעילה בהראל. יש שתי דרכים לבדוק:
        <br /><br />
        <div style={{
          background: "#F0F4FF",
          ...
          <strong>1. באזור האישי בהראל:</strong>
          <br />
          היכנסו לאזור האישי וברגע שתזדהו תראו את כל הפוליסות הפעילות.
          <br />
          <a
            href="https://www.harel-group.co.il/Pages/login-page/Login.aspx?LoginOriginatingAction=PersonalInfoPage&isshowlogin=true&Source=%2fpersonal-info%2fmy-harel%2fPages%2fclient-view.aspx&Type=PortalOTP"
            target="_blank"
            rel="noopener noreferrer"
            style={{ color: "#3D5A9E", fontWeight: 600, textDecoration: "underline" }}
          >
            כניסה לאזור האישי בהראל ←
          </a>
        </div>
        <div style={{
          background: "#F0F4FF",
          ...
          <strong>2. דרך קופת החולים:</strong>
          <br />
          חלק מפוליסות הסיעוד נרכשו דרך קופת החולים. אפשר לבדוק בטלפון <strong style={{ direction: "ltr", display: "inline-block" }}>*2700</strong> (כללית) או באזור האישי במושלם:
          <br />
          <a
            href="https://mushlam.clalit.co.il/he/gold/siudi/Pages/siudi_info.aspx"
            target="_blank"
            rel="noopener noreferrer"
            style={{ color: "#3D5A9E", fontWeight: 600, textDecoration: "underline" }}
          >
            בדיקת ביטוח סיעודי במושלם כללית ←
          </a>
        </div>
      </InfoCard>
      ...
      >
        ...
        >
          <strong>🔐 הנתונים שלכם נשמרים אך ורק אצלכם.</strong> כל מה שתמלאו בכלי הזה נשמר בדפדפן שלכם בלבד (localStorage) ואינו נשלח, נאסף או מאוחסן בשום שרת. אין לנו גישה למידע שהזנתם. הטיוטה נשמרת אוטומטית כדי שתוכלו להמשיך בזמן אחר — ותוכלו למחוק אותה בכל עת בלחיצה על "נקה טופס".
          <br /><br />
    ...
    <div style={{ animation: "fadeIn 0.4s ease" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
        <Field label="מספר ת.ז." required tooltip="9 ספרות. ניתן למצוא בתעודת הזהות">
          ...
          <div style={{ fontSize: "12px", color: "#B8860B", marginTop: "4px", lineHeight: "1.5", fontFamily: "'Noto Sans Hebrew', sans-serif" }}>
            💡 שימו לב: בנוסף למספר, יש להכין <strong>צילום תעודת זהות כולל ספח</strong> לצירוף לתביעה
          </div>
        </Field>
  ...
  const [referralContactConsent, setReferralContactConsent] = useState(false);
...
<h2>📬 דרכי שליחה</h2>
<div style="background:#E8F5EA;border:2px solid #6B8F71;border-radius:10px;padding:12px 16px;margin-bottom:14px;">
<strong style="color:#3D7A42;">💻 הגשת תביעת סיעוד מקוונת לחברת הראל (מומלץ!):</strong><br/>
<a href="https://tofes101.co.il/forms/harel-long-term-care/submit/" target="_blank" style="color:#3D7A42;font-weight:600;">https://tofes101.co.il/forms/harel-long-term-care/submit/</a><br/>
<span style="font-size:12px;color:#6A6A6A;">ממלאים פרטים ומעלים מסמכים ישירות מהמחשב או הנייד</span>
</div>
<div class="section">
    ...
    return (
      <div style={{ animation: "fadeIn 0.4s ease" }}>
        ...
        <InfoCard icon="📬" title="איך שולחים?" color="#6B8F71">
          <strong>שימו לב:</strong> הדוח האישי שהפקתם כאן הוא <strong>לשימושכם בלבד</strong> — הוא לא נשלח להראל.
          <br />
          מה שנשלח להראל הם <strong>טפסי התביעה המקוריים</strong> של הראל (או של חברת ביטוח אחרת), שמולאו ידנית, נחתמו וצורפו אליהם כל הנספחים הנדרשים.
          <br /><br />
          <strong>לאחר שמילאתם את <a
            href="https://www.harel-group.co.il/Insurance/long-term-care/claim/DocLib/%D7%98%D7%95%D7%A4%D7%A1%20%D7%94%D7%92%D7%A9%D7%AA%20%D7%AA%D7%91%D7%99%D7%A2%D7%94%20-%20%D7%91%D7%99%D7%98%D7%95%D7%97%20%D7%A1%D7%99%D7%A2%D7%95%D7%93%D7%99.pdf"
            target="_blank"
            rel="noopener noreferrer"
            style={{ color: "#3D7A42", textDecoration: "underline" }}
          >טפסי התביעה המקוריים של הראל</a>, חתמתם וצירפתם את הנספחים</strong> — אפשר לשלוח לכתובות הבאות (מומלץ בדואר רשום!):
          <br /><br />
          ...
          <br />
          📮 <strong>דואר (רצוי רשום):</strong> הראל חברה לביטוח, מח' תביעות סיעוד, אבא הלל 3, ת.ד. 10952, רמת גן 5252202
          <br /><br />
          <div style={{
            background: "#E8F5EA",
            ...
            <div>
              <strong style={{ color: "#3D7A42" }}>הגשת תביעת סיעוד מקוונת לחברת הראל (מומלץ!):</strong>
              <br />
              <a
                href="https://tofes101.co.il/forms/harel-long-term-care/submit/"
                target="_blank"
                rel="noopener noreferrer"
                style={{
                  color: "#3D7A42",
              ...
              >
                לחצו כאן להגשת התביעה באופן מקוון ←
              </a>
              <br />
              <span style={{ fontSize: "12px", color: "#6A6A6A" }}>ממלאים פרטים ומעלים מסמכים ישירות מהמחשב או הנייד</span>
            </div>
          </div>
        </InfoCard>
        ...
        <div style={{
          ...
          <div style={{ textAlign: "center", marginBottom: "16px" }}>
            ...
            <h3 style={{ fontSize: "19px", fontWeight: 800, color: "#5B4D8A", marginTop: "8px", marginBottom: "6px", fontFamily: "'Noto Sans Hebrew', sans-serif" }}>
              מעדיפים שעו"ד מומחה יטפל בתביעה עבורכם?
            </h3>
            <p style={{ fontSize: "14px", color: "#6A6A6A", lineHeight: "1.7", fontFamily: "'Noto Sans Hebrew', sans-serif" }}>
              השאירו פרטים ונחזור אליכם עם המלצה לעו"ד מתמחה בתביעות סיעוד
              <br />
          ...
          {referralSent ? (
            <div style={{ textAlign: "center", padding: "20px" }}>
              <div style={{ fontSize: "48px", marginBottom: "12px" }}>✅</div>
              <h4 style={{ fontSize: "17px", fontWeight: 700, color: "#5B4D8A", marginBottom: "8px", fontFamily: "'Noto Sans Hebrew', sans-serif" }}>
                הפרטים נשלחו בהצלחה!
              </h4>
              <p style={{ fontSize: "14px", color: "#6A6A6A", lineHeight: "1.7", fontFamily: "'Noto Sans Hebrew', sans-serif" }}>
                ניצור אתכם קשר בהקדם עם המלצה לעו"ד מתמחה.
                <br />
                לביטול או מחיקת הפנייה, פנו אלינו במייל: <strong>caresync4u@gmail.com</strong>
              </p>
            </div>
          ) : (
            <>
              {/* PRIVACY DISCLOSURE - חובת יידוע לפי סעיף 11 */}
              <div style={{
                background: "#FEFCF8",
                border: "1px solid #E0D8C8",
                borderRadius: "12px",
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
                  ...
                  <strong style={{ fontSize: "13px", color: "#3D3D3D" }}>הודעת פרטיות — יידוע לפי חוק הגנת הפרטיות</strong>
                </div>

                <strong>מהות הפנייה:</strong> פרטים שתמסרו כאן ישמשו <strong>אך ורק</strong> לצורך הפנייתכם לעו"ד המתמחה בתביעות סיעוד מול חברות ביטוח.
                <br /><br />
                <strong>מידע שיועבר:</strong> שמכם, מספר טלפון, סוג התביעה (תפקודית/קוגניטיבית) ומקום מגורים (בית/מוסד) בלבד. פרטים רגישים כגון ת.ז., מידע רפואי מפורט, פרטי בנק או מידע אחר שמילאתם בשאלון <strong>לא יועברו</strong>.
                <br /><br />
                <strong>למי יועבר:</strong> המידע יועבר ל-CareSync ומשם לעו"ד שנבחר עבורכם. המידע לא יימכר, לא יועבר לצד שלישי אחר ולא ישמש לדיוור שיווקי.
                <br /><br />
                <strong>מסירת המידע היא מרצונכם בלבד</strong> ואינה מהווה תנאי לשימוש בכלי זה. אי-מסירת פרטים תמנע הפנייה לעו"ד בלבד.
                <br /><br />
                <strong>זכויותיכם:</strong> בכל עת תוכלו לפנות ל-<strong>caresync4u@gmail.com</strong> כדי לעיין במידע שנאסף עליכם, לבקש תיקונו או מחיקתו. נפעל למילוי בקשתכם תוך 30 יום.
                <br /><br />
                <strong>שמירת המידע:</strong> פרטיכם יישמרו באופן מאובטח ויימחקו תוך 90 יום ממועד סיום הטיפול בהפנייה.
                <br /><br />
                <button
                ...
                >
                  {showPrivacyPolicy ? "הסתר מדיניות פרטיות מלאה ▲" : "מדיניות פרטיות מלאה ▼"}
                </button>
                ...
                {showPrivacyPolicy && (
                  <div style={{
                    ...
                    <strong>מדיניות פרטיות — כלי הליווי להגשת תביעת סיעוד</strong>
                    <br /><br />
                    <strong>1. בעל/ת השליטה במידע:</strong> CareSync, דוא"ל: caresync4u@gmail.com
                    <br />
                    <strong>2. סוגי מידע שנאסף:</strong> שם מלא, מספר טלפון, כתובת מייל (אם נמסרה), סוג התביעה, מקום מגורים, הערות חופשיות. אין איסוף של ת.ז., מידע רפואי מפורט, פרטי בנק או מידע ביומטרי.
                    <br />
                    <strong>3. מטרת האיסוף:</strong> הפנייה לעו"ד מתמחה בתביעות סיעוד בלבד.
                    <br />
                    <strong>4. בסיס חוקי:</strong> הסכמה מדעת של נושא המידע (סעיף 1 לחוק הגנת הפרטיות).
                    <br />
                    <strong>5. העברת מידע לצד שלישי:</strong> המידע יועבר לעו"ד מתמחים בתביעות סיעוד בלבד, שחתמו על הסכם המחייב אותם לשמור על המידע בהתאם לדין ולמחקו בתום הטיפול.
                    <br />
                    <strong>6. תקופת שמירה:</strong> המידע יישמר לתקופה של עד 90 יום ממועד סיום הטיפול בהפנייה, ולאחר מכן יימחק.
                    <br />
                    <strong>7. אבטחת מידע:</strong> המידע נשמר בכלים מאובטחים עם הרשאות גישה מוגבלות, בהתאם לתקנות הגנת הפרטיות (אבטחת מידע), התשע"ז-2017.
                    <br />
                    <strong>8. זכויות נושא המידע:</strong> זכות עיון (סעיף 13), זכות תיקון (סעיף 14), זכות מחיקה. לשם מימוש זכויותיכם, פנו ל-caresync4u@gmail.com.
                    <br />
                    <strong>9. ביטול הסכמה:</strong> ניתן לבטל את ההסכמה בכל עת באמצעות פנייה במייל. ביטול ההסכמה יביא להפסקת הטיפול בהפנייה ולמחיקת המידע.
                    <br />
                    <strong>10. הגשת תלונה:</strong> ניתן להגיש תלונה לרשות להגנת הפרטיות: www.gov.il/he/departments/the_privacy_protection_authority
                  </div>
                )}
              </div>
              ...
              <div style={{ marginBottom: "12px" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
                  <div>
                    <label style={{ fontSize: "13px", fontWeight: 600, color: "#3D3D3D", display: "block", marginBottom: "4px", fontFamily: "'Noto Sans Hebrew', sans-serif" }}>
                      שם מלא *
                    </label>
                    <input
                      type="text"
                    ...
                    />
                    <div style={{ fontSize: "11px", color: "#8A8A8A", marginTop: "2px" }}>מתוך השאלון</div>
                  </div>
                  <div>
                    <label style={{ fontSize: "13px", fontWeight: 600, color: "#3D3D3D", display: "block", marginBottom: "4px", fontFamily: "'Noto Sans Hebrew', sans-serif" }}>
                      טלפון *
                    ...
                    <input
                      type="text"
                      readOnly
                      value={formData.contactMobile || formData.mobile || ""}
                      style={{
                        ...inputStyle,
                        background: "#F0EBE0",
                        cursor: "default",
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
                  <div>
                    ...
                    <input
                      type="text"
                      readOnly
                      value={formData.claimType === "functional" ? "מוגבלות תפקודית" : formData.claimType === "cognitive" ? "תשישות נפש" : formData.claimType === "both" ? "שניהם" : "לא צוין"}
                      style={{
                        ...inputStyle,
                        background: "#F0EBE0",
                        cursor: "default",
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      readOnly
                      value={formData.residenceType === "home" ? "בבית" : formData.residenceType === "assisted" ? "דיור מוגן" : formData.residenceType === "nursing" ? "מוסד סיעודי" : "לא צוין"}
                      style={{
                        ...inputStyle,
                        background: "#F0EBE0",
                        cursor: "default",
                ...
                <div style={{ marginBottom: "12px" }}>
                  ...
                  <textarea
                    ...
                    placeholder="למשל: זמינות לשיחה, שעות מועדפות, שאלות ספציפיות..."
                    rows={2}
                    dir="rtl"
                    style={{
                      ...inputStyle,
              ...
              {/* CONSENT CHECKBOXES - הסכמה אקטיבית */}
              <div style={{ marginBottom: "16px" }}>
                <label style={{
                  display: "flex",
                  alignItems: "flex-start",
                  gap: "10px",
                  padding: "12px 14px",
                  background: referralConsent ? "#F0EAFF" : "#FEFCF8",
                  border: referralConsent ? "2px solid #7B6BA5" : "2px solid #E0D8C8",
                  borderRadius: "10px",
                  cursor: "pointer",
                  marginBottom: "8px",
                  transition: "all 0.2s",
                  ...
                  <input
                    ...
                    style={{ accentColor: "#7B6BA5", marginTop: "3px", width: "18px", height: "18px", flexShrink: 0 }}
                  />
                  <span style={{ fontSize: "13px", lineHeight: "1.7", color: "#3D3D3D", fontFamily: "'Noto Sans Hebrew', sans-serif" }}>
                    <strong>קראתי את הודעת הפרטיות ואני מסכים/ה</strong> להעברת שמי, מספר הטלפון שלי, סוג התביעה ומקום המגורים בלבד לצורך הפנייה לעו"ד מתמחה בתביעות סיעוד. ידוע לי שפרטים רגישים אחרים לא יועברו. <strong style={{ color: "#C75B3A" }}>*</strong>
                  </span>
                </label>
                ...
                <label style={{
                  ...
                  gap: "10px",
                  padding: "12px 14px",
                  background: referralContactConsent ? "#F0EAFF" : "#FEFCF8",
                  border: referralContactConsent ? "2px solid #7B6BA5" : "2px solid #E0D8C8",
                  borderRadius: "10px",
                  cursor: "pointer",
                  transition: "all 0.2s",
                }}>
                  <input
                    type="checkbox"
                    checked={referralContactConsent}
                    onChange={(e) => setReferralContactConsent(e.target.checked)}
                    style={{ accentColor: "#7B6BA5", marginTop: "3px", width: "18px", height: "18px", flexShrink: 0 }}
                  />
                  <span style={{ fontSize: "13px", lineHeight: "1.7", color: "#3D3D3D", fontFamily: "'Noto Sans Hebrew', sans-serif" }}>
                    אני מסכים/ה לקבל <strong>יצירת קשר טלפוני</strong> בנושא ההפנייה לעו"ד. <strong style={{ color: "#C75B3A" }}>*</strong>
                  </span>
                </label>
              </div>
              ...
              <div style={{ textAlign: "center" }}>
                <button
                  onClick={() => {
                    if (!referralConsent || !referralContactConsent) {
                      alert("יש לסמן את שתי תיבות ההסכמה כדי לשלוח את הפנייה.");
                      return;
                    }
                    const name = (formData.firstName || "") + " " + (formData.lastName || "");
                    const phone = formData.contactMobile || formData.mobile || "";
                    if (!name.trim() || !phone.trim()) {
                      alert("חסרים שם או טלפון. אנא חזרו לשלבים הקודמים והשלימו את הפרטים.");
                      return;
                    }
                    // In production: send to your backend / Google Form / email
                    // For now: simulate success and log to console
                    const referralData = {
                      timestamp: new Date().toISOString(),
                      name: name.trim(),
                      phone: phone.trim(),
                      claimType: formData.claimType || "not_specified",
                      residenceType: formData.residenceType || "not_specified",
                      notes: referralNotes,
                      consentGiven: true,
                      contactConsentGiven: true,
                    };
                    console.log("=== REFERRAL DATA (connect to your backend) ===", referralData);
                    setReferralSent(true);
                  }}
                  disabled={!referralConsent || !referralContactConsent}
                  style={{
                    padding: "14px 40px",
                    borderRadius: "12px",
                    border: "none",
                    background: (referralConsent && referralContactConsent)
                      ? "linear-gradient(135deg, #7B6BA5, #5B4D8A)"
                      : "#D4C9B0",
                    color: "white",
                    cursor: (referralConsent && referralContactConsent) ? "pointer" : "not-allowed",
                    fontSize: "16px",
                    fontWeight: 700,
                    fontFamily: "'Noto Sans Hebrew', sans-serif",
                    boxShadow: (referralConsent && referralContactConsent) ? "0 4px 14px rgba(91,77,138,0.3)" : "none",
                    transition: "all 0.2s",
                  }}
                >
                  ⚖️ שלחו את הפרטים שלי לעו"ד
                </button>
                <p style={{ fontSize: "11px", color: "#AAAAAA", marginTop: "8px", fontFamily: "'Noto Sans Hebrew', sans-serif" }}>
                  ניתן לבטל את ההסכמה ולבקש מחיקת המידע בכל עת — caresync4u@gmail.com
                </p>
              </div>
            </>
          )}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* DRAFT RESUME NOTICE */}
          {showDraftNotice && currentStep === 0 && (
            <div style={{
              background: "linear-gradient(135deg, #EDF5EE, #E0F0E2)",
              border: "2px solid #6B8F71",
              borderRadius: "14px",
              padding: "18px 20px",
              marginBottom: "20px",
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
                ...
                <h4 style={{ margin: 0, fontSize: "15px", fontWeight: 700, color: "#3D7A42", fontFamily: "'Noto Sans Hebrew', sans-serif" }}>
                  נמצאה טיוטה שמורה!
                </h4>
              </div>
              <p style={{ fontSize: "13px", color: "#5A5A5A", lineHeight: "1.7", marginBottom: "14px", fontFamily: "'Noto Sans Hebrew', sans-serif" }}>
                מצאנו נתונים שמילאתם בפעם הקודמת. הנתונים נשמרו בדפדפן שלכם בלבד.
                <br />
                אפשר להמשיך מאיפה שעצרתם, או להתחיל מחדש.
              </p>
              <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                <button
                ...
                <button
                  onClick={() => {
                    if (window.confirm("למחוק את כל הנתונים שמולאו ולהתחיל מחדש?")) {
                      clearDraft();
                    }
                  }}
                  style={{
          ...
          <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
            {currentStep > 0 && (
              <button
                ...
                onMouseOut={(e) => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#6A6A6A"; }}
              >
                🏠 ראשי
              </button>
            )}
            ...
            <button
              ...
              title="נקה את כל הנתונים והתחל מחדש"
              style={{
                padding: "8px 14px",
                borderRadius: "10px",
              ...
              onMouseOver={(e) => { e.currentTarget.style.background = "#FEF3F0"; e.currentTarget.style.borderColor = "#C75B3A"; }}
              onMouseOut={(e) => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.borderColor = "#E8A090"; }}
            >
              🗑️ נקה טופס
            </button>
            )}
...
root.render(React.createElement(HarelClaimWizard));
